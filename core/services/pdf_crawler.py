...
def _fetch_sitemap(start_url, timeout=30):
    ...
    for sm_url in sitemaps:
        try:
            resp = requests.get(sm_url, headers=headers, timeout=timeout)
...
def crawl_site_logic(
    start_url, output_file, max_pages=50, max_depth=1, user_agent="Bot", timeout=30
):
    """
    ...
    try:
        # Sitemap Check
        for pdf in _fetch_sitemap(start_url, timeout=timeout):
            ctx.log_pdf(pdf)
